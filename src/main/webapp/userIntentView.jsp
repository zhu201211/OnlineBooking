...
<head>
<meta charset="utf-8" />
<title>好吃吧订餐网-用户中心|我的订单--订单详情</title>
...
<nav class="U-nav Font14 FontW">
 ...
 <article class="U-article Overflow">
  	...
  	<span class="Font14 Lineheight35" ><a href="${pageContext.request.contextPath}/users/myIntent" class="FontW">我的订单</a> >> 订单详情   &nabla;</span>
   	<div class="inforlist">
	   ...
	   <span class="flow_title ">商品详情：</span>
	   <table>
	    <th>名称</th>
	    <th>数量</th>
	    <th>价格</th>
	    <th>小计</th>
	    <%for(Intent_foods ifs:foods) {%>
	    <tr>
	     <td><%=ifs.getFood().getFoodName() %></td>
	     <td><%=ifs.getNum() %></td>
	   ...
	   <span class="flow_title">送餐地址：<label > <%=intent.getUserAddress().getFrontAddress() %> <%=intent.getUserAddress().getDetailAddress() %>（<%=intent.getUserAddress().getUserName() %>收）<span class="fontcolor"><%=intent.getUserAddress().getUserPhone() %></span></label><label class="time">收货时间：<%=intent.getFinishTime() %></label></span>
